...
def make_program_addrs(table_head:str="%DW5080,270",table_name:str="PROGRAM_VIEW_TABLE-1")->dict:
    ...
    non_table_labe = table_label.split('_TABLE')[0].strip()
    addr = int(start_addr.strip()[3:-1]) # 508

    prg_lines = {}
    prg_lines[f"DW{addr}0,1,16U,0"] = f"{non_table_labe}_USESTEP"
    prg_lines[f"DW{addr}1,1,16S,-2"] = f"{non_table_labe}_PRESSSIZE"
    prg_lines[f"DW{addr}2,1,16S,-1"] = f"{non_table_labe}_PRESSINGTIME"
    ...
    for i in range(1,11):
        ...
        prg_lines[f"DW{prg_line_addr}1,1,16U,-2"] = f"{table_label}_PRG{i}_COMPENSATEDIMENSION"
        ...
        prg_lines[f"DW{prg_line_addr}4,1,16U, 0"] = f"{table_label}_PRG{i}_SELECTCAR"
        ...
        prg_lines[f"DW{prg_line_addr}9,1,16U, 0"] = f"{table_label}_PRG{i}_COMPENSATESELECT"
